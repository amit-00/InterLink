...
const Experience = ({ experience, delExperience, auth }) => {
    return (
        <div>
            {experience.map(exp => (
                <div key={exp._id} className="profile-experience mb-2 bg-light p-2">
                    { auth && <span onClick={() => delExperience(exp._id) } className="delete-edu float-right"><i className="far fa-window-close"></i></span> }
            ...
            { auth && <Link to='/add-experience' className="btn btn-light btn-sm rounded-0" ><i className="far fa-plus-square"></i> Add Experience</Link> }
        </div>
    )
}
